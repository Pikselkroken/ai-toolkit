...
interface RemoteSourceBrowseModalProps {
  /** Plugin id, e.g. "pixlstash" or "immich" */
  sourceId: string;
...
export default function RemoteSourceBrowseModal({
  ...
  const handleImportSuccess = (datasetName: string) => {
    setSuccessMessage(`"${datasetName}" downloaded successfully.`);
    onImportStarted?.();
  };
      ...
      <div className="flex flex-col" style={{ minHeight: '500px' }}>
        ...
        {selectedItem && (
          <div>
            ...
            {successMessage ? (
              <div className="p-4 border border-green-800 rounded-lg bg-green-950/40 text-sm text-green-300">
                ✓ {successMessage}
              </div>
            ) : (
              <ImportForm
